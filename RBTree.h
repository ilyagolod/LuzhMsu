...
public:
    ...
    public:
        ...
        void printNode(int lvl){
            if(_left) _left->printNode(lvl+1);
            for(int i = 0;i< lvl;i++) std::cout<<"   ";
            _key.print(); 
            if(_color==BLACK) std::cout<<"black\n";
            else std::cout<<"red\n";
            if(_right) _right->printNode(lvl + 1);
        }
    protected:
        ...
        ~Node(); // Деструктор нода гарантированно уничтожит всех потомков.
    protected:
...
protected:
    ...
    typename RBTree<Element>::Node* RBTree<Element>::Node::setLeft(Node* lf){
        ...
        Node* prevLeft = _left;
        _left = lf;
        if (prevLeft)    prevLeft->_parent = nullptr;
        return prevLeft;
    }
    ...
    void RBTree<Element>::read(std::ifstream& fp){
        std::string buf; std::string flag("");
    ...
    void RBTree<Element>::find_select(Node* roo, std::vector<Element>& obj) const{
        ...
        if(!cur) return;
	    bool key=false;
	    for(size_t i=0;i<obj.size();i++){ //очень глупый поиск, который перебирает все
    ...
    void RBTree<Element>::remove(Element *key) {
        ...
        Node* marked = deleting_node; // we mark its parent or the replacement to fix
        ...
        if (deleting_node->isRed() || (child && child->isRed())) {
            ...
            while (marked) {
                if (marked == _root) // just go away
                    marked = nullptr;
                else if (marked->isLeftChild()) {
                    Node* brother = marked->getParent()->_right; // brother is right
                    if (brother && brother->isBlack() && ((brother->getLeft() && brother->getLeft()->isRed()) || (brother->getRight() && brother->getRight()->isRed())))
                    {
                        // two cases: line and angle (just like in the insertion fix)
                        if (brother->getRight() && brother->getRight()->isRed()) // both are right, so it's the line, and it's final
                        {
                            // we perform rotation, recoloring and deletion
                            brother->_right->setBlack();
                            rotLeft(marked->_parent);
                            marked = nullptr;
                        }
                        else // now dealing with angle, making a line and exchanging colors
                        {
                            brother->setRed();
                            brother->_left->setBlack();
                            rotRight(brother);
                            // mark still stays, so we need further fixing
                        }
                    }

                    // second: the whole triplet is black
                    else if (!brother || brother->isBlack()){ // this deals with just recoloring and moving the mark
                        if (brother)
                            brother->setRed();
                        if (marked->getParent()->isBlack()) // move mark
                            marked = marked->_parent;
                        else{
                            marked->_parent->setBlack(); // it's finished
                            marked = nullptr;
                        }
                    }

                    //third: brother is red
                    else{ // rotation and recoloring
                        brother->setBlack();
                ...
                else{ // mirrored for the right side
                    Node* brother = marked->getParent()->_left; // brother is left

                    // first: if one of brother's children is red amd brother is black
                    if (brother && brother->isBlack() && ((brother->getLeft() && brother->getLeft()->isRed()) || (brother->getRight() && brother->getRight()->isRed())))
                    {
                        // two cases: line and angle (just like in the insertion fix)
                        if (brother->getLeft() && brother->getLeft()->isRed()){ // both are left, so it's the line, and it's final
                            // we perform rotation, recoloring and deletion
                            brother->_left->setBlack();
    ...
    RBTree<Element>::insertNewBstEl(Element* key) {
        ...
        if (!parent) // that means empty tree
            _root = inserted;
        else if (key<&parent->_key) // setting the mode
    ...
    RBTree<Element>::rebalanceDUG(Node* nd){
        // grandfather exists because the root is never red
        if (nd->getParent()->isLeftChild()){ // if parent is on the left side, we need right uncle
            Node* uncle = nd->getParent()->getParent()->_right;

            // if uncle doesn't exist or is black (either way we don't need it)
            if (!uncle || uncle->isBlack()){
                if (nd->isRightChild()) {// making a line
                    rotLeft(nd->_parent);
                    return nd->_left;
                }
                else{
            ...
            else {// uncle exists and is red
                // parent and uncle black
                nd->_parent->setBlack();
                uncle->setBlack();
                nd->getParent()->_parent->setRed(); // grandfather red
        ...
        else{ // parent is on the right side
            Node* uncle = nd->getParent()->getParent()->_left;
            if (!uncle || uncle->isBlack()){
                if (nd->isLeftChild()){ // make a line
                    rotRight(nd->_parent);
                    return nd->_right; // need to check what happens now (because we didn't do any recoloring and the problem stays)
                }
                else{
                    nd->_parent->setBlack();
                    nd->getParent()->_parent->setRed();
                ...
                nd->getParent()->_parent->setRed(); // grandfather red
                // need to check what happens to grandfather
                return nd->getParent()->_parent;
            }
        }
    ...
    void RBTree<Element>::rebalance(Node* nd){
        // while parent is red, we need to fix
        while (nd->isDaddyRed())
            nd = rebalanceDUG(nd);
        _root->setBlack(); // root must always be black after everything
    }
    ...
    void RBTree<Element>::rotLeft(typename RBTree<Element>::Node* nd){
        ...
        if (y->_left) // cut node nd.l.r and make it nd's left child
            nd->setRight(y->_left);
        else{
            nd->setRight(nullptr);
            y->_parent = nullptr;
        ...
        if (nd == _root){
            ...
            y->_parent = nullptr;
            y->setLeft(nd);
        }// change root if the pivotal point is root

        else {// set the left child in place
            // set nd.l as child of nd.p
            if (nd->isRightChild())
                nd->_parent->setRight(y);
            else
                nd->_parent->setLeft(y);
            y->setLeft(nd); // set nd as child
        }
    }
    ...
    void RBTree<Element>::rotRight(typename RBTree<Element>::Node* nd){
        ...
        if (y->_right) // cut node nd.l.r and make it nd's left child
            nd->setLeft(y->_right);
        ...
        if (nd == _root){
            ...
            if (nd->isLeftChild())
                ...
                nd->_parent->setRight(y);
            y->setRight(nd); // set nd as child
        }
    }

    template <typename Element>
    void RBTree<Element>::print(int lvl) const{
        if(!_root) std::cout<< "no elements"<< std::endl;
        else{
            if(_root->_left) _root->_left->printNode(lvl+1);
            for(int i = 0;i< lvl;i++) std::cout<<"   ";
            _root->_key.print(); 
            if(_root->_color==BLACK) std::cout<<"black ";
            else std::cout<<"red\n";
            if(_root->_right)_root->_right->printNode(lvl + 1);
        }
        //через BFS можно
        std::cout<<std::endl;
        return;
